...
beta = 1.0/(temperature * kb_in_eV_pre_K)  # inverse thermal energy for calibration in 1/eV
...
def train_model(key, model, train_data, valid_data, num_epochs, learning_rate, forces_weight, batch_size):
    ...
    params = model.init(init_key,
                        ...
                        dst_idx=dst_idx,
                        src_idx=src_idx)
    #old_optimizer opt_state = optimizer.init(params)
    
    #clean_optimizer 
    schedule = optax.exponential_decay(
    init_value=learning_rate,
    transition_steps=200,     # number of batches before decay
    decay_rate=0.95,           # decay rate
    staircase=True
    )

    optimizer = optax.chain(
        optax.clip_by_global_norm(1.0),  # clip gradients with norm > 1
